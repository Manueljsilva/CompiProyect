...
void ImpCodeGen::visit(VarDec* vd) {
  for (auto varInit : vd->varinits) {
    ...
    ventry.dir = current_dir;
    ventry.is_global = process_global;
    direcciones.add_var(varInit->name, ventry);
  }
  return;
}
...
int ImpCodeGen::visit(UnaryExp* e) {
  ...
  VarEntry ventry = direcciones.lookup(idExp->name);

  // Generar la instrucción load para cargar el valor de la variable
  if (ventry.is_global)
    codegen(nolabel, "load", ventry.dir);
  else
    codegen(nolabel, "loadr", ventry.dir);

  // Generar la instrucción para incrementar o decrementar
  ...
  // Generar la instrucción store para almacenar el nuevo valor en la variable
  if (ventry.is_global)
    codegen(nolabel, "store", ventry.dir);
  else
    codegen(nolabel, "storer", ventry.dir);
